            ...
            <div className={styles.cardBody}>
                <div className={styles.buyerSellerRow}>
                    <div className={styles.buyerBlock}>
                        ...
                        <span className={styles.inn}>ИНН {application.company?.inn}</span>
                    </div>
                ...
                <div className={styles.dateChecksRow}>
                    <span className={styles.date}>
                        {`${formatDate(application.date?.start)} → ${formatDate(application.date?.end)}`}
                    </span>
            ...
            return (
                <div className={styles.emptyState}>
                    ...
                    <span className={styles.hint}>
                        Здесь можно искать заявки по номеру, названию компании или ИНН.
                    </span>
                </div>
            );
        }

        if (localSearchQuery && applications.length === 0) {
            return (
                <div className={styles.emptyState}>
                    <SearchIcon2 />
                    <span>Ничего не найдено</span>
                    <span className={styles.hint}>
                        Заявок с такими параметрами нет. Проверьте ваш запрос.
                    </span>
                </div>
            );
        }
            ...
            <div className={`${styles.overlay} ${isClosing ? styles.closing2 : ''}`} onClick={handleClose}>
                ...
                >
                    ...
                    <div className={styles.results}>
                        {renderEmptyState()}
                        {localSearchQuery && applications.length > 0 && 
                            applications.map((application, index) => renderCard(application, index))
                        }
                    </div>
                </div>
            </div>
